...
__all__ = ["LargeWeightMatrixAnime", "animate_large_heatmap"]
...
class LargeWeightMatrixAnime(Scene):
    ...
    def construct(self):
        ...
        max_label = (
            DecimalNumber(v_max, num_decimal_places=1).scale(0.7).next_to(color_bar, UP)
        )
        min_label = (
            DecimalNumber(v_min, num_decimal_places=1)
            .scale(0.7)
            .next_to(color_bar, DOWN)
        )
        ...
        for i in range(1, steps):
            ...
            self.play(
                current_img.animate.become(new_img), run_time=self.dt, rate_func=linear
            )


def animate_large_heatmap(
    matrix_stack,
    dt=0.2,
    resolution=(1920, 1080),
    file_name="large_wt_anim",
    title_str="",
):

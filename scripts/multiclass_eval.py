...
plt.figure()
plt.boxplot([df["train_accuracy"], df["test_accuracy"]], tick_labels=["Train", "Test"])
plt.title("Accuracy")
plt.savefig(os.path.join(data_dir, f"{keyword}_accuracy_boxplot_{timestamp}.png"))

plt.figure()
plt.boxplot([df["train_f1"], df["test_f1"]], tick_labels=["Train", "Test"])
plt.title("F1 Score")
...
plt.figure()
plt.boxplot(
    [df["train_precision"], df["test_precision"]], tick_labels=["Train", "Test"]
)
plt.title("Precision")
plt.savefig(os.path.join(data_dir, f"{keyword}_precision_boxplot_{timestamp}.png"))

plt.figure()
plt.boxplot([df["train_recall"], df["test_recall"]], tick_labels=["Train", "Test"])
plt.title("Recall")
